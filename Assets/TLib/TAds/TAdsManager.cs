...
using System.IO;
using DG.Tweening;
public class TAdsManager : MonoBehaviour
{
    ...
    public void GetADS()
    {
        ...
        if (LIST_ADS == null)
        {
            GetHTML(link, (TAdsManager.hamyes)OnLoadDone, this);
        }
        else
        {
            ShowAds();
    ...
    public void ParseCONTENT(string STR, bool is_saved)
    {
        ...
        string[] STRA = STR.Split('|');
        int NUM = STRA.Length / 3;
        LIST_ADS = new List<AAdsLabelItem>();
        try
        {
            for (int i = 0; i < NUM; i++)
            {
                ...
                LIST_ADS.Add(new AAdsLabelItem(STRA[index + 1], link, STRA[index + 2], this));
    ...
    public void ShowAds(int r = -1)
    {
        ...
        //AAdsLabelItem ads = LIST_ADS[CURENT_ADS];
        ...
        TWTAds found = FindObjectOfType<TWTAds>();
        if(found!=null)
        {
            found.UpdateAds();
        }
...
public class AAdsLabelItem
{
    ...
    MonoBehaviour M;
    public AAdsLabelItem(string text, string link, string link_image, MonoBehaviour m)
    {
        M = m;
        TEXT = text;
        LINK = link;
